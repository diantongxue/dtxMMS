...
import PAGE_MERCHANT_PREVIEW_BTN04 from './PAGE_MERCHANT_PREVIEW_BTN04/PAGE_MERCHANT_PREVIEW_BTN04';
import PAGE_MERCHANT_PREVIEW_BTN05 from './PAGE_MERCHANT_PREVIEW_BTN05/PAGE_MERCHANT_PREVIEW_BTN05';
import PAGE_MERCHANT_PREVIEW_BTN06 from './PAGE_MERCHANT_PREVIEW_BTN06/PAGE_MERCHANT_PREVIEW_BTN06';
import PAGE_MERCHANT_PREVIEW_BTN07 from './PAGE_MERCHANT_PREVIEW_BTN07/PAGE_MERCHANT_PREVIEW_BTN07';
import PAGE_MERCHANT_PREVIEW_BTN08 from './PAGE_MERCHANT_PREVIEW_BTN08/PAGE_MERCHANT_PREVIEW_BTN08';
import PAGE_MERCHANT_PREVIEW_BTN09 from './PAGE_MERCHANT_PREVIEW_BTN09/PAGE_MERCHANT_PREVIEW_BTN09';
...
interface ElectronAPI {
  ...
  browserView: {
    ...
    openFullscreen: (id: string) => Promise<{ success: boolean; error?: string }>;
    openInTab: (id: string) => Promise<{ success: boolean; error?: string }>;
...
const MerchantPreview: React.FC = () => {
  ...
  const updateNavigationState = async (browserId: string) => {
    ...
    try {
      ...
      // 忽略错误
      // 遵循宪法.md第13.1.1节TypeScript规范：禁止使用any类型，使用unknown类型
    }
  };
  ...
  // 监听页面可见性变化和tab切换，当页面隐藏或不在当前tab时隐藏 BrowserView
  useEffect(() => {
    if (!electronAPI?.browserView) return;

    // 检查当前组件是否在可见的tab中（通过检查DOM元素是否可见）
    const isComponentVisible = () => {
      if (!browserPreviewRef.current) return false;
      // 检查父元素是否有active类（tab切换时active类会变化）
      const parentPanel = browserPreviewRef.current.closest('[class*="tabPanel"]');
      if (parentPanel) {
        const hasActiveClass = parentPanel.className.includes('active');
        if (!hasActiveClass) return false;
      }
      
      const rect = browserPreviewRef.current.getBoundingClientRect();
      // 检查元素是否在视口中且可见（宽度和高度大于0表示可见）
      return (
        rect.width > 0 &&
        rect.height > 0 &&
        !document.hidden
    ...
    const handleVisibilityChange = () => {
      ...
      if (currentBrowserViewIdRef.current) {
        // 如果组件不可见或页面隐藏，隐藏 BrowserView
        electronAPI.browserView.setVisibility(currentBrowserViewIdRef.current, visible).catch(() => {
          // 忽略错误
        });
      }
    };
    ...
    // 使用MutationObserver监听DOM变化（tab切换时class会变化）
    const observer = new MutationObserver(() => {
      // 使用requestAnimationFrame确保在DOM更新后检查
      requestAnimationFrame(() => {
        handleVisibilityChange();
      });
    });

    // 观察父元素的class变化（tab切换时会改变active class）
    if (browserPreviewRef.current?.parentElement) {
      observer.observe(browserPreviewRef.current.parentElement, {
        attributes: true,
    ...
    // 使用ResizeObserver监听容器大小变化（tab切换时容器可能被隐藏，大小变为0）
    ...
    // 使用IntersectionObserver监听元素是否在视口中
    ...
    if (browserPreviewRef.current) {
      intersectionObserver = new IntersectionObserver(
        (entries) => {
          ...
          if (currentBrowserViewIdRef.current) {
            electronAPI.browserView.setVisibility(currentBrowserViewIdRef.current, isVisible && !document.hidden).catch(() => {
              // 忽略错误
            });
          }
        },
        { threshold: 0.01 } // 只要1%可见就认为可见
      );
      intersectionObserver.observe(browserPreviewRef.current);
    }
  ...
  // 处理全屏打开（遵循宪法.md第6节错误处理规范：所有异步操作必须有错误处理）
  const handleOpenFullscreen = async () => {
    ...
    try {
      const result = await electronAPI.browserView.openFullscreen(currentBrowserViewIdRef.current);
      if (!result.success) {
        console.error('打开全屏窗口失败:', result.error);
        // 可以显示错误提示
      }
    } catch (error: unknown) {
      // 遵循宪法.md第13.1.1节TypeScript规范：禁止使用any类型，使用unknown类型
      const err = error as { message?: string };
      console.error('打开全屏窗口时发生错误:', err.message || error);
    }
  };
        ...
        <div className={styles.browserPreview} ref={browserPreviewRef}>
          {currentBrowser?.status === 'crashed' ? (
            <div className={styles.crashPrompt}>
              ...
              <PAGE_MERCHANT_PREVIEW_BTN08 onClick={() => handleRecover(currentBrowser.id)}>
                恢复浏览器实例
              </PAGE_MERCHANT_PREVIEW_BTN08>
            </div>
          ) : (
            <>
              ...
              <div className={styles.browserToolbar} ref={toolbarRef}>
                <div className={styles.toolbarLeft}>
                  <PAGE_MERCHANT_PREVIEW_BTN02 onClick={handleBack} disabled={!canGoBack} />
                  <PAGE_MERCHANT_PREVIEW_BTN03 onClick={handleForward} disabled={!canGoForward} />
                  <PAGE_MERCHANT_PREVIEW_BTN04 onClick={handleRefresh} />
                </div>
                ...
                <div className={styles.toolbarRight}>
                  <PAGE_MERCHANT_PREVIEW_BTN05 onClick={handleQuickExtract} />
                  <PAGE_MERCHANT_PREVIEW_BTN06 onClick={() => handleQuickAction('refresh')} />
                  <PAGE_MERCHANT_PREVIEW_BTN07 onClick={handleShowStats} />
                  {isElectron && (
                    <PAGE_MERCHANT_PREVIEW_BTN09
                      onClick={handleOpenFullscreen}
                      disabled={!currentBrowserViewIdRef.current}
                    />
                  )}
                </div>
              </div>
